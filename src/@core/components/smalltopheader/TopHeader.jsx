...
import { loadLanguages, selectCurrentLanguage, selectLanguages, setCurrentLanguage } from "src/store/reducers/languageSlice";
import { sysConfigdata } from "src/store/reducers/settingsSlice";
import CustomHoverDropdown from "../Common/CustomHoverDropdown";
import { notificationApi } from "src/store/actions/campaign";
                            ...
                            <div className="login__sign__form">
                                {isLogin() && checkUserData(userData) ? (
                                    <DropdownButton onMouseEnter={handlersOne.onMouseEnter}
                                        ...
                                        <Dropdown.Item onClick={() => router.push("/profile")}>{t("Profile")}</Dropdown.Item>
                                        ...
                                        <div>
                                            {!guestlogout ?
                                                <div className="right_guest_profile">
                                                    <img className="profile_image" onClick={(e) => profileGuest(e)} src="/images/profileimages/6.svg" alt="profile" />
                                                    <DropdownButton onMouseEnter={handlersOne.onMouseEnter}
                                                        onMouseLeave={handlersOne.onMouseLeave}
                                                        show={menuOpenOne}
                                                        id="dropdown-basic-button" title={`${t("Hello Guest")}`} className="dropdown__login" >
                                                        <Dropdown.Item onClick={() => router.push("/guest-profile")}>{t("Profile")}</Dropdown.Item>
                                                    </DropdownButton>
                                        ...
                                        notifications.map((data, key) => {
                                            return (
                                                <div key={key} className="outer_noti">
                                                    <img className="noti_image" src={data.image ? data.image : "/images/user.svg"} alt="notication" id="image" onError={imgError}/>
